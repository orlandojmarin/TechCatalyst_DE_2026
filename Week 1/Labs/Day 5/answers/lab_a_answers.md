...
| What are the zones, left to right (ingest, store, transform, serve)? | Ingest: steps 1-5 (user queries flow through API Gateway → Kinesis → Firehose → Lambda compress → S3). Store: steps 6-7 (S3 raw logs, Parquet, and clustered output, with Glue crawlers cataloging each). Transform: steps 8-13 (Glue consolidation job, K-means clustering, Athena top-N query, Bedrock classification). Serve: steps 14-15 (DynamoDB → Lambda → API Gateway back to the search page and business analysts). |
| Name the services in each zone | Ingest (steps 1-5): Amazon OpenSearch, API Gateway, Kinesis Data Streams, Data Firehose, Lambda (compress). Store (steps 6-7): S3 (three stages: raw, Parquet, clustered), Glue Catalog (crawlers at steps 7, 9, 11). Transform (steps 8-13): AWS Glue jobs (steps 8, 10), Athena (step 12), Step Functions (orchestration), Lambda, Bedrock (step 13). Serve (steps 14-15): DynamoDB, Lambda, API Gateway. |
