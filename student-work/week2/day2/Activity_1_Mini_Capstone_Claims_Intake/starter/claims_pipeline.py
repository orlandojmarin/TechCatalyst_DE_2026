...
def parse_money(raw):
    ...
    clean_string = clean.replace(",", "")

    try:
        clean_string_float = round(float(clean_string), 2)
        is_numeric = True
    except ValueError:
        is_numeric = False

    if clean_string == "" or clean_string == "N/A" or is_numeric == False or clean_string < 0:
        return None
    
    clean_string_float = float(clean_string)
