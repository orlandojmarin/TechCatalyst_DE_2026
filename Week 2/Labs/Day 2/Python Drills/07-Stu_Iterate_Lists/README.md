...
Copy `Unsolved/claims_cashflow.py` to your project and complete the following:
...
Optional, for fast finishers. Add these to your script. See `Advanced Track/` for
the full modern-Python drills.

### Walrus operator (`:=`)

Find the first business day on which the running cumulative cash flow crosses 500.
The walrus operator updates and tests the running total in one expression.

```python
total = 0
for i, day_net in enumerate(daily_net, start=1):
    if (total := total + day_net) > 500:
        print(f"day {i}: cumulative {total} crossed 500")
        break
```

Expected: `day 4: cumulative 734 crossed 500`.

### Generator

Write a generator that yields only the deficit days, then materialize it with
`list(...)`.

```python
def deficit_days(net):
    for d in net:
        if d < 0:
            yield d

print("deficits:", list(deficit_days(daily_net)))
```

Expected: `deficits: [-224, -544, -650, -43, -123, -151, -311]`.
