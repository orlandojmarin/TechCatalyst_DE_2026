...
Copy `Unsolved/loss_ratio.py` to your project and complete the following steps:
...
print(summarize(1200, 5000, 800))           # total: 7000
print(summarize(1200, 5000, label="payouts"))  # payouts: 6200

def make_claim(**fields):
    return fields

print(make_claim(claim_id="CLM-1", reserve=5000))
```

Then practice the `import` system: move `calculate_loss_ratio` into a new file
`helpers.py` in the same folder, and in your main script write
`from helpers import calculate_loss_ratio` and call it. Splitting code into modules
is how projects stay organized as they grow.
