...
Copy `Unsolved/status_banner.py` to your project and complete the following steps.
...
## Stretch: break and continue

`continue` skips to the next item; `break` stops the loop early. Add this and
...
Expected: prints 1200, then 5000, then "found a large payout, stopping" (the zeros
are skipped, and the loop stops at the first payout over 4000).
