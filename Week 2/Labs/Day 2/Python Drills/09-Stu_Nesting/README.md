...
Copy `Unsolved/Core/payment_rollup_core.py` to your project and complete the following:
...
Optional, for fast finishers. After the Core append, write a generator that yields
the claims whose total paid exceeds a threshold. Use the walrus operator to compute
each total once. See `Advanced Track/` for the full modern-Python drills.

```python
def big_claims(payments, threshold):
    for claim_id, records in payments.items():
        if (total := sum(r[2] for r in records)) > threshold:
            yield claim_id, total

print("big (>3000):", list(big_claims(claim_payments, 3000)))
```

Expected: `big (>3000): [('CLM-3002', 5000.0), ('CLM-3004', 5000.0)]`.
